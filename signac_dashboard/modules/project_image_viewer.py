...
class ProjectImageViewer(Module):
    """Displays images in the project directory.
    ...
    def get_cards(self, project):
        ...
        image_globs = [
            glob.iglob(project.root_directory() + os.sep + image_glob)
            for image_glob in self.img_globs
        ]
        image_files = itertools.chain(*image_globs)
        for filepath in image_files:
            yield make_card(os.path.relpath(filepath, project.root_directory()))
